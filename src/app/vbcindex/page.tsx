...
export default async function VBCIndexPage() {
  ...
  return (
    <div className="bg-white min-h-screen">
      {/* Hero / Intro */}
      <section className="pt-40 pb-48 bg-white border-b border-brand-neutral overflow-hidden relative">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-24 items-center">
            <div className="lg:col-span-7">
              <div className="flex items-center gap-4 mb-12">
                <span className="px-4 py-1.5 bg-brand-plum/5 text-brand-plum text-[10px] font-bold uppercase tracking-[0.3em] rounded-full border border-brand-plum/10">Product Spotlight</span>
                <div className="h-px w-12 bg-brand-plum/20" />
              </div>
              <h1 className="text-6xl lg:text-8xl mb-12 tracking-tight">
                {vbcIndexPage.headline}
              </h1>
              <p className="text-3xl text-brand-plum/50 leading-relaxed max-w-2xl italic border-l-4 border-brand-gold pl-12">
                {vbcIndexPage.intro}
              </p>
            </div>
      ...
      {/* Product Pillars */}
      <section className="py-32 lg:py-48 bg-brand-neutral/20">
        <div className="max-w-7xl mx-auto px-6 lg:px-12">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {pillars.map((pillar, i) => (
              <div key={i} className="brand-card group border-none shadow-subtle hover:shadow-bold">
                <div className="w-14 h-14 bg-brand-neutral/50 rounded-xl flex items-center justify-center mb-10 group-hover:bg-brand-plum transition-colors duration-500 shadow-sm">
                  <pillar.icon className="w-7 h-7 text-brand-plum group-hover:text-white transition-colors" />
                </div>
                <h3 className="text-2xl mb-6 text-brand-plum font-bold group-hover:translate-x-1 transition-transform">{pillar.title}</h3>
                <p className="text-brand-plum/60 text-lg leading-relaxed italic">{pillar.description}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-32 lg:py-48 bg-white border-y border-brand-neutral">
        <div className="max-w-7xl mx-auto px-6 lg:px-12">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-24 lg:gap-40">
            <div className="space-y-16">
              <div>
                <h2 className="text-[10px] font-bold uppercase tracking-[0.5em] text-brand-plum/40 mb-10">{vbcIndexPage.labels.problem}</h2>
                <p className="text-4xl lg:text-5xl font-display font-bold text-brand-dark leading-tight italic">
            ...
            <div className="space-y-16 lg:pt-24">
              <div>
                <h2 className="text-[10px] font-bold uppercase tracking-[0.5em] text-brand-plum/40 mb-10">{vbcIndexPage.labels.approach}</h2>
                <p className="text-2xl text-brand-plum/60 leading-relaxed italic border-l-4 border-brand-gold pl-12">
      ...
      {/* Why It Matters */}
      <section className="py-48 lg:py-64 bg-brand-dark text-white overflow-hidden relative">
        <div className="absolute top-0 right-0 w-1/3 h-full bg-brand-plum opacity-30 skew-x-[-12deg] translate-x-20" />
        <div className="max-w-5xl mx-auto px-6 lg:px-12 text-center relative z-10">
          <h2 className="text-[11px] font-bold uppercase tracking-[0.6em] text-brand-gold mb-16">{vbcIndexPage.labels.whyItMatters}</h2>
          <p className="text-4xl lg:text-[5.5rem] mb-24 tracking-tight leading-[1.1] italic font-display">
